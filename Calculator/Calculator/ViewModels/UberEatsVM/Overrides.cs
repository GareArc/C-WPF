...
namespace Calculator
{
    partial class UberEatsVM
    {
        public override BasicItem CreateBasicItem()
        {
            ...
            return new BasicItem(price, quantity, taxed, GLOBAL.UBEREATS_SERVICE_WEIGHT, ShopName);
        }
        ...
        public override SharedItem CreateSharedItem()
        {
            ...
            // Update item info and create new item
            globalVM.AddedNewItem = false;
            return new SharedItem(price, quantity, taxed, GLOBAL.UBEREATS_SERVICE_WEIGHT, 
                GetRelationType(), ChoiceList_Relation[SeletedIndex_Relation], ShopName);
        }
        public override void Confirm(object parameter)
        {
            globalVM.UberEatsTip = double.Parse(Tip);
            globalVM.UberEatsOther = double.Parse(Other);
            CloseWindow();
        }
    }
}
